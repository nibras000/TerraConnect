...
const Saved2 = () => {
    ...
    const handleSavedButton = async()=>{
      ...
      try{
          const result = await axios.post('http://localhost:3001/properties/deleteSaved',temp)
          window.alert("Removed from saved")
          navigate('/')
      }catch(error){
          console.log("error")
      }
      ...
      <div className='buy2-outer-container'>
        <div className='buy2-inner-container'>
          ...
          <div className='saved-button-class'>
            <input type="submit" value="Remove from saved" name="saved" class="saved-button" onClick={handleSavedButton}/>
          </div>
        </div>
      </div>
    );
