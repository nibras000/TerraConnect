  ...
  useEffect(() => {
    async function fetchData() {
        try {
            ...
            const temp = await axios.get(`http://localhost:3001/properties/saved/?user_id=${user_id}`);
...
const handleClick = (index) => {
  const user_id = localStorage.getItem('user_id');
  if(user_id) {
     navigate("/saved/more", { state: index }); 
     console.log(index,"hi")
  } else {
      window.alert("You are not logged in");
      console.log(index,"hi")
      navigate("/saved/more", { state: index }); // Need to delete this
  }
};

  return (
    <div className={`saved-container ${searchResult.length === 0 ? 'empty-style' : ''}`} style={{ height: searchResult.length === 0 ? '100vh' : 'auto' }}>
                <sp className="saved-results">Saved Results</sp>
